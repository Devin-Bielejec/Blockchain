...
class Blockchain(object):
    ...
    def new_transaction(self, sender, recipient, amount):
        ...
        #A new block would be len(self.chain) + 1 
        print(self.chain)
...
def transactions_new():
    ...
    if not all(k in data for k in required):
        response = {
            "message": "Recipient, sender, or amount missing"
        }
...
def transactions_id(id):
    transactions = []
    for block in chain:
        current_transactions = block["transactions"]

        filtered_transactions = [trans for trans in current_transactions if trans["recipient"] or trans["sender"] == id]
        
        transactions.append(filtered_transactions)
